#!/usr/bin/env python3
"""DG-C ACTIVE_OPEN_PORT_INGRESS_NO_FIRMWARE_RX_CONTINUOUS peer.
...
Every admitted valid C1UD causes exactly one immediate synchronous sendto of
the same bytes.  There is no peer timer, pacing, sleep, queue, batch, or retry.
"""
...
BLOCKED_ADMISSION_SEQUENCE_MISS = "BLOCKED_DG_C_ADMISSION_SEQUENCE_MISS"
BLOCKED_EVIDENCE = "BLOCKED_DG_C_EVIDENCE_CONTRACT_INVALID"
BLOCKED_ASYNC = "BLOCKED_DG_C_PEER_UDP_ASYNC_ERROR"
SEND_FAIL = "DG_C_PEER_INGRESS_SEND_FAIL"
...
W5500_UNKNOWN_FIELDS = (
    "W5500_PACKET_ARRIVAL",
    "W5500_SOCKET_MATCH_ACCEPTANCE",
    "W5500_RX_STORAGE",
    "W5500_RX_BUFFER_SATURATION",
    "W5500_MATCHED_PORT_INTERNAL_SEMANTICS",
)


def submission_to_proven_open_port(success_total: int, evidence_valid: bool) -> str:
    """Return only the peer-application treatment claim supported by evidence."""
    if not evidence_valid:
        return "UNKNOWN"
    return "1" if success_total > 0 else "0"
...
class DgCController:
    ...
    def summary_lines(self) -> list[str]:
        ...
        lines = [
            ...
            "PEER_APPLICATION_SUBMISSION_TO_PROVEN_OPEN_PORT="
            + submission_to_proven_open_port(e.ingress_success_total, True),
            f"PEER_FATAL_REASON={e.fatal_reason or 'NONE'}",
            f"PEER_RESULT={self.result()}",
        ]
        lines.extend(f"{name}=UNKNOWN" for name in W5500_UNKNOWN_FIELDS)
        return lines
...
def _fixture_controller(fake: FakeSocket | None = None) -> tuple[DgCController, FakeSocket]:
    fake = fake or FakeSocket()
    return DgCController(fake.sendto, fake.getsockname()), fake
...
def run_self_test() -> int:
    ...
    check("destination_exact_open_port", fake.sent[0][1] == ("192.168.50.10", 50001))
    check("runtime_destination_never_50002", INGRESS_DESTINATION[1] != 50002)
    controller.observe(build_frame(1), EXPECTED_SOURCE)
    check("exactly_one_send_per_valid_admitted", len(fake.sent) == 2)
    check("second_send_same_exact_bytes", fake.sent[1][0] == build_frame(1))
    ...
    check("attempt_success_semantics", controller.evidence.ingress_attempt_total == controller.evidence.ingress_success_total == 2)
    check("peer_result_positive_allowlist", controller.result() == "PASS")
    check("blocked_result_closed_world", recv_error.result() == "BLOCKED")
    check("treatment_claim_one", submission_to_proven_open_port(1, True) == "1")
    check("treatment_claim_zero", submission_to_proven_open_port(0, True) == "0")
    check("treatment_claim_unknown", submission_to_proven_open_port(9, False) == "UNKNOWN")
    summary = controller.summary_lines()
    check(
        "summary_submission_to_open_port_one",
        "PEER_APPLICATION_SUBMISSION_TO_PROVEN_OPEN_PORT=1" in summary,
    )
    check(
        "w5500_unknown_never_promoted",
        all(f"{name}=UNKNOWN" in summary for name in W5500_UNKNOWN_FIELDS),
    )
...
def _write_summary(controller: DgCController) -> None:
...
def run_live(args: argparse.Namespace) -> int:
    ...
    controller: DgCController | None = None
    ...
    try:
        try:
            ...
            controller = DgCController(lambda _p, _d: 0, ("NA", 0))
        ...
        try:
            ...
            _write_summary(controller)
            return EXIT_BLOCKED_CONTROL
        controller = DgCController(sock.sendto, (str(actual[0]), int(actual[1])))
