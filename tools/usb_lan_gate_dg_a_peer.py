#!/usr/bin/env python3
"""Gate DG-A receive-only UDP peer with three-state admission and offline fixtures.

This is a new, additive file for Gate DG-A (Mode 17, USB_FIXED10_UDP_TX_RX_POLL_EMPTY).
DG-A isolates a single changed variable against the Gate C1 (Mode 15) accepted
baseline: an empty udp.parsePacket() poll on the device side, with no udp.read() and
no peer-application UDP return traffic. So this peer reuses the Gate C1 wire format
(C1UD, gate 1) verbatim -- it does not invent a new frame format -- but reuses the
Gate C2 three-state admission machine (BOUND_NOT_ARMED -> ARMED_WAIT_SEQUENCE_ZERO ->
ADMITTED) from tools/usb_lan_gate_c2_peer.py, adapted to drop echo entirely: this peer
never calls a socket send/sendto/sendmsg/connect anywhere. It does not import or
modify tools/usb_lan_gate_peer.py (the Gate C1 peer) or tools/usb_lan_gate_c2_peer.py
(the Gate C2 peer); both are deliberately left untouched. See
docs/usb-lan-gate-dg-a-contract.md for the canonical requirements this file
implements (three-state peer admission, receive-only, PEER_TX_TO_DEVICE_TOTAL
invariant, BLOCKED_ADMISSION_SEQUENCE_MISS).
"""
...
# N-02 receive-only proof, part 3: forbidden substrings that would indicate a
# transmit-capable code path. Checked against this file's own source by the
# self-test (see check_no_transmit_path below) -- not just asserted in prose.
_FORBIDDEN_TRANSMIT_SUBSTRINGS = (".sendto(", ".send(", ".sendmsg(", ".connect(")


class FrameError(ValueError):
    """Base class for rejected Gate DG-A (C1UD) frames."""
...
class PacketValidator:
    """Parses/validates C1UD frames and tracks sequence integrity.

    Deliberately parallel to the Gate C1 and Gate C2 peers' PacketValidator
    (same field semantics), but never sequence-zero-gates admission itself --
    that policy lives one layer up in DgAAdmissionController.
    """
...
class DgAAdmissionController:
    """Three-state peer admission machine, receive-only (no echo).

    BOUND_NOT_ARMED: every packet is logged only; no counting, no transmit.
    ARMED_WAIT_SEQUENCE_ZERO: the first fully-valid frame decides admission.
      ...
      (a distinct classification, not a DG-A network FAIL; no automatic retry).
    ADMITTED: strict per-packet validation only. This peer never sends anything
      back to the device on any state transition or packet -- there is no send
      channel, no dispatch method, and no code path that reaches a socket send
      call anywhere in this file (N-02 receive-only proof).

    passed() -- B-09: reads only underlying state/counters, never PEER_COMPLETE or
    PEER_RESULT (those are serialized *from* this method's result in
    summary_lines(), never read back into it -- a strictly one-directional flow).
    """

    def __init__(self, validator: PacketValidator) -> None:
        ...
        # Informational only -- never required to be zero for Layer-3 PASS.
        self.bound_not_armed_packet_count = 0
        # PRE_ADMISSION_NON_C1_COUNT: packets observed while
        # ARMED_WAIT_SEQUENCE_ZERO that are not a fully-valid C1UD frame from
        # the expected source. These never touch the strict PacketValidator
        # counters that gate the final Peer PASS contract. Informational only.
        self.pre_admission_non_c1_count = 0
    ...
    def _try_fully_valid_c1(self, packet: bytes, source: tuple[str, int]) -> Frame | None:
        # Pure, non-mutating check: does NOT touch validator/tracker state,
        # so a non-C1UD or wrong-source packet observed pre-admission never
        # poisons the strict counters (PRE_ADMISSION_NON_C1_COUNT tracks it
        # separately instead).
    ...
    def observe(self, packet: bytes, source: tuple[str, int], arrival: float) -> dict[str, object]:
        ...
        if self.state is AdmissionState.ARMED_WAIT_SEQUENCE_ZERO:
            frame = self._try_fully_valid_c1(packet, source)
            if frame is None:
                self.pre_admission_non_c1_count += 1
                return {
            ...
            # Fully valid C1UD frame, sequence 0: admit, then this same packet
            # becomes the first strict-validator packet.
...
def summary_lines(controller: DgAAdmissionController) -> list[str]:
    ...
    return [
        ...
        f"PRE_ADMISSION_NON_C1_COUNT={controller.pre_admission_non_c1_count}",
        f"MAX_INTERARRIVAL_MS={maximum:.6f}",
        f"P99_INTERARRIVAL_MS={percentile_99(summary.inter_arrivals):.6f}",
        # Structural invariant, not a measured counter: this file has no
        # send/sendto/sendmsg/connect call anywhere (see
        # _FORBIDDEN_TRANSMIT_SUBSTRINGS / check_no_transmit_path). This does not
        # claim zero Ethernet activity of any kind (ARP etc. is out of scope) --
        # only that this peer application never transmits a UDP datagram to the
        # device.
...
def check_no_transmit_path() -> list[str]:
    """N-02 receive-only proof, part 3 (source grep, saved as evidence).

    Scans this file's own source text for any of the forbidden transmit
    substrings. Returns the list of matches (empty means clean). The
    docstrings/comments above deliberately spell these substrings with a
    leading '.' and trailing '(' so this scan does not just check for the
    English word "send" (which appears legitimately in comments) but for an
    actual method-call shape.
    """
    source_text = Path(__file__).read_text(encoding="utf-8")
    # Exclude this function's own body (and the constant above) from the scan,
    # since it legitimately must name the forbidden substrings to look for them.
    lines = source_text.splitlines()
    scan_text = "\n".join(
        line
        for line in lines
        if "_FORBIDDEN_TRANSMIT_SUBSTRINGS" not in line
        and "check_no_transmit_path" not in line
    )
    # Drop this function's own body by re-splitting on the function boundary.
    marker = "def check_no_transmit_path"
    body_start = scan_text.find(marker)
    if body_start != -1:
        next_def = scan_text.find("\ndef ", body_start + 1)
        scan_text = scan_text[:body_start] + (scan_text[next_def:] if next_def != -1 else "")
    return [needle for needle in _FORBIDDEN_TRANSMIT_SUBSTRINGS if needle in scan_text]
...
def run_self_test() -> int:
    ...
    # --- N-02 part 3: receive-only source scan (no send/sendto/sendmsg/connect) ---
    transmit_matches = check_no_transmit_path()
    check("receive_only_no_transmit_path", len(transmit_matches) == 0, ",".join(transmit_matches))
    ...
    expect_error("gate_error", mutate_and_recrc(valid, 5, 2), GateError)
    ...
    # --- wrong source / wrong source port (PacketValidator) ---
    expected_source = ("192.168.50.10", DEFAULT_PORT)
    wrong_ip_validator = PacketValidator(*expected_source)
    wrong_ip_row = wrong_ip_validator.observe(build_frame(0, 0), ("192.168.50.11", DEFAULT_PORT), 0.0)
    ...
    # --- BOUND_NOT_ARMED: valid C1UD packet -> strict counters untouched, no transmit ---
    validator = PacketValidator(*expected_source)
    controller = DgAAdmissionController(validator)
    for sequence in range(3):
        row = controller.observe(build_frame(sequence, sequence * 20000), expected_source, sequence * 0.02)
        check(f"bound_not_armed_suppressed[{sequence}]", row["state"] == "BOUND_NOT_ARMED")
    ...
    # --- BOUND_NOT_ARMED: garbage/non-C1 packet -> strict counters untouched ---
    ...
    arm_controller = DgAAdmissionController(arm_validator)
    ...
    arm_controller.arm()
    check("arm_is_idempotent", arm_controller.state is AdmissionState.ARMED_WAIT_SEQUENCE_ZERO)

    # --- ARMED_WAIT_SEQUENCE_ZERO: pre-admission non-C1 noise -> untouched, waiting ---
    noise_validator = PacketValidator(*expected_source)
    noise_controller = DgAAdmissionController(noise_validator)
    noise_controller.arm()
    old_c2_frame = bytearray(build_frame(0, 0))
    old_c2_frame[0:4] = b"C2UD"
    old_c2_frame[5] = 2
    old_c2_frame[30:32] = crc16_ccitt_false(bytes(old_c2_frame[:30])).to_bytes(2, "big")
    old_c2_frame = bytes(old_c2_frame)
    old_row = noise_controller.observe(old_c2_frame, expected_source, 0.0)
    check(
        "armed_wait_non_c1_packet_log_only",
        old_row["state"] == "ARMED_WAIT_SEQUENCE_ZERO",
    )
    check("armed_wait_non_c1_pre_admission_count", noise_controller.pre_admission_non_c1_count == 1)
    check(
        "armed_wait_non_c1_never_poisons_strict_counters",
        noise_validator.summary.rx_total == 0
        and noise_validator.summary.format_error == 0
        and noise_controller.state is AdmissionState.ARMED_WAIT_SEQUENCE_ZERO,
    )
    garbage_row = noise_controller.observe(b"\x01" * FRAME_LENGTH, expected_source, 0.01)
    check(
        "armed_wait_garbage_pre_admission_non_c1",
        garbage_row["state"] == "ARMED_WAIT_SEQUENCE_ZERO"
        and noise_controller.pre_admission_non_c1_count == 2
        and noise_validator.summary.rx_total == 0,
    )

    # --- ARMED_WAIT_SEQUENCE_ZERO: first valid C1UD, sequence != 0 -> BLOCKED ---
    blocked_validator = PacketValidator(*expected_source)
    blocked_controller = DgAAdmissionController(blocked_validator)
    ...
    check(
        ...
        followup_row["state"] == BLOCKED_ADMISSION_SEQUENCE_MISS,
    )
    check("admission_nonzero_no_pre_admission_non_c1", blocked_controller.pre_admission_non_c1_count == 0)

    # --- ARMED_WAIT_SEQUENCE_ZERO: first valid C1UD, sequence == 0 -> ADMITTED ---
    seq0_validator = PacketValidator(*expected_source)
    seq0_controller = DgAAdmissionController(seq0_validator)
    seq0_controller.arm()
    seq0_controller.observe(build_frame(0, 0), expected_source, 0.0)
    ...
    check("admission_sequence_zero_becomes_first_strict_packet", seq0_validator.summary.first_sequence == 0)
    ...
    recovery_controller = DgAAdmissionController(recovery_validator)
    recovery_controller.arm()
    recovery_controller.observe(old_c2_frame, expected_source, 0.0)
    ...
    check(
        ...
        and recovery_controller.pre_admission_non_c1_count == 2,
    )
    recovery_controller.observe(build_frame(0, 100000), expected_source, 0.02)
    check(
    ...
    # --- ADMITTED: valid sequence stream -> Layer-3 PASS-compatible ---
    stream_validator = PacketValidator(*expected_source)
    stream_controller = DgAAdmissionController(stream_validator)
    stream_controller.arm()
    for sequence in range(10):
        stream_controller.observe(build_frame(sequence, sequence * 20000), expected_source, sequence * 0.02)
    check("admitted_valid_stream_passes", stream_controller.passed())
    check("admitted_valid_stream_peer_tx_zero", summary_lines(stream_controller)[-2] == "PEER_TX_TO_DEVICE_TOTAL=0")

    # --- ADMITTED: bad CRC after admission -> strict failure ---
    crc_validator = PacketValidator(*expected_source)
    crc_controller = DgAAdmissionController(crc_validator)
    crc_controller.arm()
    crc_controller.observe(build_frame(0, 0), expected_source, 0.0)
    corrupted = bytearray(build_frame(1, 20000))
    corrupted[30] ^= 0x01
    crc_controller.observe(bytes(corrupted), expected_source, 0.02)
    check("admitted_bad_crc_strict_failure", crc_validator.summary.crc_error == 1 and not crc_controller.passed())

    # --- ADMITTED: wrong length -> strict failure ---
    length_validator = PacketValidator(*expected_source)
    length_controller = DgAAdmissionController(length_validator)
    length_controller.arm()
    length_controller.observe(build_frame(0, 0), expected_source, 0.0)
    length_controller.observe(build_frame(1, 20000)[:-1], expected_source, 0.02)
    check("admitted_wrong_length_strict_failure", length_validator.summary.length_error == 1 and not length_controller.passed())

    # --- ADMITTED: bad payload -> strict failure ---
    payload_validator = PacketValidator(*expected_source)
    payload_controller = DgAAdmissionController(payload_validator)
    payload_controller.arm()
    payload_controller.observe(build_frame(0, 0), expected_source, 0.0)
    payload_controller.observe(mutate_and_recrc(build_frame(1, 20000), 16, 0), expected_source, 0.02)
    check("admitted_bad_payload_strict_failure", payload_validator.summary.payload_error == 1 and not payload_controller.passed())

    # --- ADMITTED: wrong source IP -> strict failure ---
    source_ip_validator = PacketValidator(*expected_source)
    source_ip_controller = DgAAdmissionController(source_ip_validator)
    source_ip_controller.arm()
    source_ip_controller.observe(build_frame(0, 0), expected_source, 0.0)
    source_ip_controller.observe(build_frame(1, 20000), ("192.168.50.99", DEFAULT_PORT), 0.02)
    check("admitted_wrong_source_ip_strict_failure", source_ip_validator.summary.unexpected_source == 1 and not source_ip_controller.passed())

    # --- ADMITTED: wrong source port -> strict failure ---
    source_port_validator = PacketValidator(*expected_source)
    source_port_controller = DgAAdmissionController(source_port_validator)
    source_port_controller.arm()
    source_port_controller.observe(build_frame(0, 0), expected_source, 0.0)
    source_port_controller.observe(build_frame(1, 20000), (expected_source[0], 50002), 0.02)
    check("admitted_wrong_source_port_strict_failure", source_port_validator.summary.source_port_error == 1 and not source_port_controller.passed())

    # --- ADMITTED: wrong flags -> strict failure ---
    flags_validator = PacketValidator(*expected_source)
    flags_controller = DgAAdmissionController(flags_validator)
    flags_controller.arm()
    flags_controller.observe(build_frame(0, 0), expected_source, 0.0)
    flags_controller.observe(mutate_and_recrc(build_frame(1, 20000), 6, 0x01), expected_source, 0.02)
    check("admitted_wrong_flags_strict_failure", flags_validator.summary.flags_error == 1 and not flags_controller.passed())

    # --- ADMITTED: sequence gap / duplicate / out-of-order -> strict failure ---
    seq_validator = PacketValidator(*expected_source)
    seq_controller = DgAAdmissionController(seq_validator)
    seq_controller.arm()
    ...
    seq_controller.observe(build_frame(1, 40000), expected_source, 0.04)
    check("admitted_duplicate_detected", seq_validator.summary.duplicate == 1 and not seq_controller.passed())
    gap_row = seq_controller.observe(build_frame(4, 60000), expected_source, 0.06)
    check("admitted_gap_detected", gap_row["gap"] == 2 and seq_validator.summary.seq_gap == 2)
    ooo_row = seq_controller.observe(build_frame(0, 80000), expected_source, 0.08)
    check("admitted_out_of_order_detected", ooo_row["out_of_order"] == 1)

    # --- B-09: passed() never reads its own serialized output (PEER_COMPLETE/PEER_RESULT) ---
    b09_validator = PacketValidator(*expected_source)
    b09_controller = DgAAdmissionController(b09_validator)
    b09_controller.arm()
    for sequence in range(3):
        b09_controller.observe(build_frame(sequence, sequence * 20000), expected_source, sequence * 0.02)
    result_before_serialization = b09_controller.passed()
    summary_lines(b09_controller)  # forces PEER_COMPLETE/PEER_RESULT to exist as serialized output
    result_after_serialization = b09_controller.passed()
    check(
        "passed_is_non_self_referential",
        result_before_serialization == result_after_serialization and result_before_serialization,
    )
...
def run_sink(args: argparse.Namespace) -> int:
    """Live networked receive-only mode.

    Not exercised by this implementation task (network bind/UDP traffic is out
    of scope); provided so the DG-A runner has a real target to invoke during a
    future authorized physical trial. This function never sends anything to the
    device -- there is no sendto/send/sendmsg/connect call anywhere in this
    file (see check_no_transmit_path, exercised by --mode self-test).
    """
    ...
    print(
        f"PEER_READY=1 MODE=sink BIND={args.bind_ip}:{args.port} STATE=BOUND_NOT_ARMED",
        file=sys.stderr,
        flush=True,
    )
    try:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", required=True, choices=("self-test", "sink"))
    ...
    if args.mode == "sink":
        if not args.bind_ip:
            parser.error("sink mode requires --bind-ip")
        if not args.expected_source_ip:
            parser.error("sink mode requires --expected-source-ip")
        if not args.arm_file or not args.armed_file:
            parser.error("sink mode requires --arm-file and --armed-file")
...
def main() -> int:
    args = parse_args()
    return run_self_test() if args.mode == "self-test" else run_sink(args)
