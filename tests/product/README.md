...
The first byte is buttons, not a report-ID prefix. The candidate rejects the callback's hasReportId=true form, but UHS1.7.0 does not automatically derive that flag from the report descriptor (`hidcomposite.h` records this limitation). Do not claim complete descriptor verification from that flag. The2026-09-06 passive intake independently captured the connected unit's116-byte descriptor and confirmed no Report ID item and64-bit input. It also identified Button15 at byte2 bit7, which remains unmapped pending physical identification; do not call that bit reserved. Descriptor padding and byte7 are ignored. Hat0..7 is directional and0x0F is observed neutral; other low-nibble values are rejected. See [intake evidence](../../docs/validation/product-usb-intake-20260906.md); manual mapping remains pending.
